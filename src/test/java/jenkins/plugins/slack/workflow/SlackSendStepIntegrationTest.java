...
public class SlackSendStepIntegrationTest {
    ...
    public void test_global_config_override() throws Exception {
        ...
        //just define message
        job.setDefinition(new CpsFlowDefinition("slackSend(message: 'message', webhookId: 'webhookId', token: 'token', tokenCredentialId: 'tokenCredentialId', channel: '#channel', color: 'good');", true));
        ...
        jenkinsRule.assertLogContains(Messages.SlackSendStepConfig(false, false, false), run);
    }
    ...
    public void test_fail_on_error() throws Exception {
        WorkflowJob job = jenkinsRule.jenkins.createProject(WorkflowJob.class, "workflow");
        //just define message
        job.setDefinition(new CpsFlowDefinition("slackSend(message: 'message', webhookId: 'webhookId', token: 'token', tokenCredentialId: 'tokenCredentialId', channel: '#channel', color: 'good', failOnError: true);", true));
